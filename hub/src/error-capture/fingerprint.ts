...
export function normalize(s: string): string {
  ...
  // length >= 8 AND at least one actual hex LETTER (a-f) so a pure decimal
  // run (epoch-ms timestamps, byte counts) is never collapsed — a digit
  // alone is not evidence of hex, since every decimal number contains one.
  // Ordinary hex-letter English words ("decade", "facade") stay protected by
  // the length>=8 floor.
  out = out.replace(/\b[0-9a-f]{8,}\b/gi, (m) => (/[a-f]/i.test(m) ? 'HEX' : m));
