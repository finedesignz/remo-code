...
It runs **strictly after** the gate list (cost-cap / threshold / dedupe / rate-limit stay non-bypassable — a gated repair never spawns). Leak-safety: it uses the same hub-authoritative `reserveSessionSlot` → `createRun` → `session.start` sequence as the web/start endpoints, with `endRun` + `releaseSessionSlot` on send failure and a per-session in-flight lock. EVERY outcome frees the slot on a bounded, connection-independent timescale: at capacity / no supervisor creates no run; createRun/send failure releases; and on **timeout** it proactively `endRun(spawn_on_error_timeout)` + `releaseSessionSlot` before returning false, then falls back to park/skip. The proactive timeout-close is load-bearing because the spawn-on-error run has `session_id=null` — the in-band close-by-session path can never reap it, so without the proactive close a silently-dropped `session.start` would strand an open NULL-session run against the concurrency cap until WS-close or the 24h reconnect sweep (the known `at_capacity` orphan-run leak class). With it, the bound is the timeout (minutes), not 24h. The per-session in-flight lock is **in-process** (single `bun` process; prod is single-replica on Coolify); if the hub is ever scaled >1 replica it needs a cross-process spawn guard (DB advisory lock). Flag OFF reproduces today's behaviour exactly.
