# Architecture Notes (CLAUDE.md overflow archive)

> **Why this file exists:** the per-subsystem and per-phase rollups below were
> moved out of `CLAUDE.md` (2026-05-29) to keep the always-loaded `CLAUDE.md`
> under the harness performance threshold. This file is **not** auto-loaded —
> it is indexed by context-mode and read on demand. Each subsystem also has a
> dedicated `docs/*.md`; this archive preserves the original CLAUDE.md prose and
> cross-cutting invariants in one place.
>
> **Same-commit-update contract still applies:** when you change a subsystem,
> update its dedicated `docs/<subsystem>.md` AND the matching section here if the
> invariant text changed. `CLAUDE.md` only carries the pointer.
...
Durable rules first, then per-subsystem rollups (each carries its own "update this section in the same commit" contract):

- **Foundations:** Architecture · Commands · Local Supervisor · Database · WebSocket Protocol · Key Design Decisions · Environment Variables · Deployment · Releases · PR Hygiene · API docs convention
- **Cross-cutting deep modules:** Hub shared dispatch + webhook-intake modules (the `hub/src/dispatch/` + `hub/src/webhooks/intake.ts` pipeline all inbound subsystems ride)
- **Subsystem rollups:** Scheduled Tasks · Error Capture · Grid View · Orchestrator Session
- **Phase rollups (historical, still load-bearing):** Phase 05 Codex+Rootless · Phase 06 Coolify Self-Heal · Phase 07 Titanium Auth Cutover · Phase 08 Revanote · Phase 11 Structured Workflows · Phase 12 Mobile Tauri shell + 12.1 hub enablement · Phase 12 UI restructure · Phase 12 Telegram Bridge
...
## Phase 12: Mobile Tauri Client — shell (mobile/tauri/)
...
Tables: `users` (identity + Titanium `license_status` mirror; legacy `password_hash` retained behind `ALLOW_LEGACY_LOGIN` — see Phase 07), `auth_sessions` (opaque cookie sessions, Phase 07), `sessions` (Claude Code sessions), `messages` (chat history), `api_keys` (supervisor authentication). All queries are scoped by `user_id` with explicit WHERE clauses.
...
- Auth: `{ type: "auth", token }` → opaque session cookie (Phase 07); legacy JWT verified via `JWT_SECRET` accepted only when `ALLOW_LEGACY_LOGIN=true`
...
- Subscription quota is polled by the **supervisor**, not the hub — the OAuth access token lives only in `~/.claude/.credentials.json` on the dev machine and never leaves it. The supervisor poll (`supervisor/src/usage/oauth-poll.ts`, wired in `hub-client.ts` on a 5-min interval from `auth_ok`) reads the local token, hits `GET https://api.anthropic.com/api/oauth/usage` (headers ported from the ClaudeUsage psm1: `anthropic-beta: oauth-2025-04-20`, `User-Agent: claude-code/2.0.15`, Accept/Content-Type), and forwards the parsed **four** utilization windows — `five_hour`, `seven_day`, and (Claude Max only) `seven_day_opus` + `seven_day_oauth_apps`, each `{ utilization 0-100, resets_at ISO8601 }` — via the `usage_report` agent message. **Only the parsed windows cross the wire; the OAuth token is never serialized to the hub.** Hub keeps a per-user in-memory snapshot (`hub/src/usage/store.ts`) and rebroadcasts to web clients via WS event `subscription_usage` (also surfaced on `GET /api/usage/summary` → `claude_window`). The web header strip (`UsageStrip.tsx`) and Settings → Usage "Anthropic limit windows" card (`UsageTab.tsx`) render util% + an Opus pill + a `resets_at − now` countdown (colour bands <50 / <80 / ≥80). Requires supervisor ≥0.7.0 (the poll ships only with the new MSI).
...
- `web/src/pages/SettingsPage.tsx` + `web/src/pages/settings/{ConnectionsTab,CredentialsTab,UsageTab,ProfileTab}.tsx` — `#/settings` route. **4 tabs** (milestone v-settings-overhaul, 2026-05): the old `PromptsTab` and `OrchestratorTab` are deleted — `?tab=prompts` and `?tab=orchestrator` redirect to Connections. The orchestrator is now a pinned special top row in the Connections repo table (enable/disable/start/stop in-row); root-folder paths moved out of Connections into the supervisor first-run wizard.
...
- **Design-token discipline (per `~/.claude/design-preferences.md`):** `rounded-xl` for cards/dialogs, `rounded-lg` for inputs/buttons/list items, `rounded` for chips. NEVER `rounded-2xl` or higher. NO `shadow-2xl` / `shadow-lg` / `shadow-md` — use `ring-1 ring-white/5` instead. Accent button text uses `text-[var(--text-on-accent)]`, never `text-white`. Red destructive buttons may keep `text-white`. **App accent migrated indigo→blue (orange = CTA-only; never indigo — `web/test/no-indigo.test.ts` guards); see `~/.claude/design-preferences.md`.**
...
- Web (milestone v-settings-overhaul, 2026-05): the standalone `OrchestratorTab` is **deleted**. Orchestrator enable/disable/start/stop now live as a pinned special top row in the Connections repo table (`web/src/pages/settings/ConnectionsTab.tsx` / `SupervisorPage`), and `?tab=orchestrator` redirects to Connections. The `/api/orchestrator` endpoints are unchanged — only the UI relocated. Sidebar still pins the orchestrator row to position 0 (accent ring — blue, not indigo — + `Orchestrator` pill); GridPage still enforces orchestrator-in-cell-0 when the orchestrator session is a member of the active tab.
...
## Phase 12.1: Mobile Tauri Client — hub-side enablement
