...
import { getStoredToken, clearAuth } from '../lib/auth.ts'
...
export function useWebSocket(token: string | null) {
  ...
  const connect = useCallback(() => {
    ...
    if (!authToken) {
      // No token — don't open the WS at all. Surfaces a clean disconnected
      // state and lets the app shell redirect to login. Prevents the
      // "open → 4001 → reconnect" loop from ever starting on first paint
