  ...
  if (!expanded) {
    return (
      <Tooltip label={`Root folders (${items.length})`}>
        {/* Outer button is the REAL 44x44 hit target (transparent — the visible
            pill is the smaller inner span below); `-m-2.5` collapses its flow
            footprint to 24px without shrinking the actual click area, same
            technique as every other icon action in this row. */}
        <button
          type="button"
          onClick={toggleExpanded}
          aria-label={`Root folders (${items.length})`}
          className="group flex items-center justify-center min-w-[44px] min-h-[44px] -m-2.5 rounded-lg"
        >
          <span className="relative flex items-center justify-center w-9 h-9 rounded-lg text-[var(--text-muted)] bg-[var(--bg-tertiary)]/40 group-hover:bg-[var(--bg-tertiary)]/60 group-hover:text-[var(--text-primary)] transition-colors">
            {/* Badge anchors to this inner 20px box (not the 44px hit target) so it
                stays fully inside the visible pill bounds and never clips
                against a sibling in the flex row — was `-top-1 -right-1` off
                the 44px box. */}
