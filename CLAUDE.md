...
| Inbound-email work (`remo_work`) | [remo-work.md](docs/remo-work.md) | Milestone WORK — `POST /api/ext/work` (+ `GET /api/ext/work/:id`): an inbound CLIENT EMAIL → the repo's stream-json session → fix under the site's dir → **QC (build + HTTPS deploy-verify) BEFORE any publish** → publish ONLY on a site carrying `auto_publish` (**defaults FALSE** ⇒ PR + preview + report). The email is the LEAST-trusted input in the system: it is `fenceUntrusted`d (shared fence), the result envelope is **nonce'd** (`<<WORK:{nonce}>>`, LAST match, fails closed — no prose fallback), and `finalizeWork` ANDs the agent's `published` claim with the site's flag IN SQL. Two default-EMPTY trust tables: `work_repo_allowlist` (audit F6 — a repo not on it ⇒ 403, no dispatch, no spend) and `work_sites` (`client_emails` sender allowlist ⇒ 403 `unknown_sender`; `site_dir` = write blast radius). Audit trail `work_runs` (F9: source email + FULL prompt + commit SHAs + QC evidence). Gates: threshold → cost cap → **token cap** → human-only-PTY (server-inferred actor `external-work`) → `workRateGate` (`REMO_WORK_MAX_PER_HOUR`, default 4) → `workRepoAllowlistGate`. Scope `ext:work` (NOT satisfied by `ext:ask`). Reaper `REMO_WORK_MAX_MS` (45min). MCP: `remo_work`/`remo_get_work`. |
...
- **Untrusted inbound text NEVER reaches production trust.** An inbound client email
  (`/api/ext/work`) is the least-authenticated input in the system, and it points an agent with
  file-write powers at a LIVE CLIENT WEBSITE. Three defaults hold that line and **all three are
  OFF/EMPTY on a fresh install**: (1) `work_repo_allowlist` is EMPTY — a repo not on it is 403'd
  before any row is inserted or any token spent (audit F6); (2) `work_sites.client_emails` is the
  sender allowlist — an unknown `source.from` never reaches a session; (3) `work_sites.auto_publish`
  DEFAULTS FALSE — a site without it gets fix + QC + PREVIEW + report and **never touches
  production**. Publishing is additionally gated on QC (build + a real HTTPS deploy-verify) passing
  FIRST. The email text is fenced with the SHARED `fenceUntrusted`; the result envelope is NONCE'D
  and fails closed; and `finalizeWork` ANDs the agent's `published` claim with the site flag IN SQL,
  so the hub never takes the agent's word for what went live. Never relax one of these "just for a
  test" — see [docs/remo-work.md](docs/remo-work.md) §threat-model and the proofs in
  `hub/test/ext-work-containment.test.ts`.
