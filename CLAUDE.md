...
- **Default target falls back to the orchestrator (orchestrator-autolaunch, 2026-05-28).** Inbound `dispatchInbound` resolves the effective target as `telegram_default_session_id || <user's open orchestrator session>`. So Telegram always tracks the orchestrator unless the user explicitly picks another default. On fallback the orchestrator id is **lazy-pinned** into `telegram_default_session_id` so the OUTBOUND bridge (which matches on the column) forwards the reply too. A **stale pin** (the explicit default points at a now-deleted session, verified via `getSession`) is self-healed: it's dropped and re-resolved to the orchestrator + re-pinned. If the orchestrator session isn't live when a message arrives, dispatch returns `agent_offline` → the existing autoheal (`runDoctor` → orchestrator-aware `launchSessionForUser` → `launchOrchestrator`) launches it and replays the buffered message. `prewarmAfterLink` no longer pins a project session when the orchestrator is enabled — it leaves the default unset so the fallback wins. When the orchestrator is disabled/explicitly-disabled and no default is set → the original "No default session" reply.
